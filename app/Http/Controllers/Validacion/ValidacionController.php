<?php namespace SICPA\Http\Controllers\Validacion;
...
use SICPA\Http\Requests\IniciarSesionRequest;
use SICPA\Http\Requests\RecuperarContrasenaRequest;
use SICPA\Usuario;
...
class ValidacionController extends Controller {
	...
	*/
	...
	{
		$pregunta = $request->get('usu_preg');
		$respuesta = $request->get('usu_rpta');
		$email = $request->get('email');
		$usuario = Usuario::where('email','=',$email) -> first();

		if($pregunta === $usuario->usu_preg && $respuesta === $usuario->usu_rpta)
		{
			$contrasena = $request ->get('password');
			$usuario->password = bcrypt($contrasena);
			$usuario->save();

			return redirect('/validacion/inicio')->withErrors(['recuperada'=>'La contraseña se cambió.']);
		}
		return redirect('/validacion/recuperar')->withInput($request->only('email','usu_preg'))->withErrors(['usu_preg'=>'La pregunta y/o respuesta ingreaadas no coinciden.']);
	}
